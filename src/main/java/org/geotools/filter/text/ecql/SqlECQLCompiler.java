...
public class SqlECQLCompiler extends ECQLParser implements
	 ...
	 * @param filterFactory
	 */
	public SqlECQLCompiler(final String txtSource,
	...
	/**
	 * compile source to produce a Filter. The filter result must be retrieved
	 * with {@link #getFilter()}.
	 * 
	 * @throws CQLException
	 */
	public void compileFilter() throws CQLException {
		try {
			...
			throw new CQLException(tokenError.getMessage(),
					getTokenInPosition(0), this.source);
		} catch (CQLException e) {
			throw e;

		} catch (ParseException e) {
			throw new CQLException(e.getMessage(), getTokenInPosition(0),
					e.getCause(), this.source);
		}
	}
	...
	private Object build(Node n) throws CQLException {
		...
		// ----------------------------------------
		case JJTINTEGERNODE:
			return this.builder.buildLiteralInteger(getTokenInPosition(0)
					.toString());
		case JJTFLOATINGNODE:
			return this.builder.buildLiteralDouble(getTokenInPosition(0)
					.toString());
		...
		case JJTNOT_IN_PREDICATE_NODE:
			Or orFilter = this.builder
					.buildInPredicate(JJTEXPRESSION_IN_LIST_NODE);
